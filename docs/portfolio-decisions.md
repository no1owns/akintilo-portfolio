...
## 2026-09-20 — AppOmni/Secureframe separation: already adequate, not changed

**Finding, not a new decision:** `brand-systems-and-web-performance.json` already separates its AppOmni and Secureframe content with two in-page `sectionHeading` blocks ("Secureframe" then "AppOmni"), and its `outcomes` array already attributes each metric to the correct company. This satisfies the brief's requirement to keep the two clearly labeled within their shared case study. No structural change was made.

One inconsistency was flagged, not fixed: the project's `overview` text and `outcomes` order lead with AppOmni, while the body's section order is Secureframe-then-AppOmni (which matches employment chronology — Secureframe 2022–2024, AppOmni Dec 2024–present). Reordering either the prose or the sections would be a copy/structure edit, so it's logged in `ROADMAP.md` under "Items requiring Ayodeji's decision" rather than changed unilaterally.
