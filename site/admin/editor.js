...
const blocks=field('Page sections','blocks','list',{required:false,collapsed:true,summary:'{{type}} · {{title}}',types:[
 ...
 {label:'Gallery',name:'gallery',widget:'object',fields:[title,field('Columns','columns','select',{options:['1','2','3'],default:'2'}),field('Images','images','list',{fields:image,summary:'{{alt}}',min:1}),spacing]},
 ...
 {label:'Video or embed link',name:'video',widget:'object',fields:[field('Video title','title'),field('Embed URL','url','string',{hint:'Use a YouTube /embed/ or Vimeo player URL. Other HTTPS embeds open as a link.',pattern:['^https://','Use an HTTPS URL']}),spacing]}
]});
...
if(!local&&!auth){document.getElementById('setup').innerHTML='<h1>Portfolio editor</h1><p>The editor is installed. GitHub sign-in needs to be connected before publishing edits from this address.</p><p>The repository setup guide explains the OAuth connection and the VITE_CMS_AUTH_URL setting.</p><p><a href="https://github.com/no1owns/akintilo-portfolio">Open setup guide on GitHub</a></p><a href="/">Back to portfolio</a>';}
 ...
 CMS.registerPreviewTemplate('projects',({entry,getAsset})=>{const p=getData(entry);return h('main',{className:'preview'},h('p',{className:'eyebrow'},p.category),h('h1',{},p.title),h('p',{className:'summary'},p.summary),h('p',{},[p.company,p.role].filter(Boolean).join(' / ')),p.cover&&h('img',{src:img(p.cover,getAsset),alt:p.coverAlt||''}),h('h2',{},'The work'),...paragraphs(p.overview),h('div',{className:'outcomes'},...(p.outcomes||[]).map((o,i)=>h('div',{key:i},h('strong',{},o.value),h('p',{},o.label)))),...(p.blocks||[]).map((b,i)=>h('section',{key:i,className:'block space-'+(b.spacing||'regular')},b.title&&h('h2',{},b.title),b.type==='text'?paragraphs(b.body):b.type==='gallery'?h('div',{className:'gallery columns-'+b.columns},...(b.images||[]).map((im,j)=>h('figure',{key:j},h('img',{src:img(im.image,getAsset),alt:im.alt||''}),h('figcaption',{},im.caption)))):b.type==='split'?h('div',{className:'split image-'+b.side},h('img',{src:img(b.image,getAsset),alt:b.alt||''}),h('div',{},...paragraphs(b.body))):b.type==='image'?h('figure',{className:b.width==='narrow'?'narrow':''},h('img',{src:img(b.image,getAsset),alt:b.alt||''}),h('figcaption',{},b.caption)):h('p',{},'Video / embed: '+(b.url||'')))));});
